...
class WeatherDay {
    constructor(data, arr) {
        ...
        if (!data && arr)
            data = arr[0];
        this.date = new Date(data.startTime);
        this.temperature = data.temperature;
        this.temperatureUnit = data.temperatureUnit;
        this.shortForecast = data.shortForecast;
        this.icon = data.icon;
        this.relativeHumidity = ((_a = data.relativeHumidity) === null || _a === void 0 ? void 0 : _a.value) + "%";
        this.windDirection = data.windDirection;
        this.windSpeed = data.windSpeed;
        this.dow = this.date.toLocaleDateString('en-US', { weekday: 'short' });
        this.probabilityOfPrecipitation = (((_b = data.probabilityOfPrecipitation) === null || _b === void 0 ? void 0 : _b.value) || ((_c = data.relativeHumidity) === null || _c === void 0 ? void 0 : _c.value)) + "%";
        // is hourly so filter out the odds to get the 2 hour periods
        if (arr)
            this.hours = arr.filter(d => d.number % 2 === 0).map(d => new WeatherDay(d, null));
    }
}
class WeatherDayOfWeek {
    constructor(data) {
        this.date = new Date(data.startTime),
            this.id = data.id,
            this.windSpeed = data.windSpeed,
            ...
            this.probabilityOfPrecipitation = data.probabilityOfPrecipitation.value,
            this.dewpoint = data.dewpoint.value,
            this.relativeHumidity = data.relativeHumidity.value;
        const dow = this.date.toLocaleDateString('en-US', { weekday: 'short' });
        // get month
        const mon = this.date.toLocaleDateString('en-US', { month: 'short' });
        this.dow = dow.substring(0, 3);
        this.mon = mon;
...
class Location {
    constructor(data) {
        this.name = data.name;
        this.latitude = data.relativeLocation.geometry.coordinates[0];
        this.longitude = data.relativeLocation.geometry.coordinates[1];
        this.city = data.relativeLocation.properties.city;
        this.state = data.relativeLocation.properties.state;
    }
    getAnyAlerts() {
        return fetchJson(`https://api.weather.gov/alerts/active?area=${this.state}`);
    }
}

let LocalWeather = class LocalWeather extends s {
    constructor() {
        super(...arguments);
        this.forecast = [];
    }
    ...
    getWeather(lat, lon) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.location = new Location(data.properties);
            const { forecastHourly, forecast } = data.properties;
            const hourly = yield fetchJson(forecastHourly);
            this.today = new WeatherDay(null, hourly.properties.periods);
            console.log('----------today----------');
            console.log(this.today);
            const fore = yield fetchJson(forecast);
            this.forecast = fore.properties.periods.map((day) => new WeatherDayOfWeek(day))
                .reduce((acc, day, i) => {
                if (i === 0)
                    return acc;
                if (day.name.includes('Night')) {
                    acc[acc.length - 1].night = day;
                }
                else {
                    acc.push(day);
                }
            ...
            console.log(fore.properties.periods);
            console.log('----------forecast----------');
            console.log(this.forecast);
            /* this.location.getAnyAlerts().then((data) => {
                console.log(data);
            }); */
        });
    }
			...
			<div class="w-full max-w-screen-sm bg-white p-10 rounded-xl ring-8 ring-white ring-opacity-40">
				<div class="flex justify-between">
					<div class="flex flex-col">
						<span class="text-2xl font-bold">${(_a = this.today) === null || _a === void 0 ? void 0 : _a.dow}</span>
...
__decorate([
    r()
], LocalWeather.prototype, "forecast", void 0);
__decorate([
    r()
], LocalWeather.prototype, "location", void 0);
LocalWeather = __decorate([
    t('local-weather')
], LocalWeather);
